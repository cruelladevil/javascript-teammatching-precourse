...
- [x] Lib
  ...
  - [x] 파일 시스템 md 파일을 불러오는 기능
  - [x] 배열을 셔플하는 기능

- [x] 과정을 저장하는 기능#Course

  - [x] 과정을 저장한다.
  - [x] 5단계로 나누어져 있는 레벨. ( 프론트 백엔드 공통 )

- [x] 크루원#Crew

  - [x] 이름을 가지고 있음
  - [x] 매칭 이력을 저장하고 있음
  - [x] 레벨에 따른 매칭 이력을 가져오는 기능

- [x] 크루원과 코스가 상호작용하는 부분#CourseCrewController

  - [x] 프론트 엔드 크루원을 초기화 하는 기능
  - [x] 백엔드 크루원을 초기화 하는 기능
...
- [ ] 페어 매칭#PareMatching
  ...
  - [ ] 매칭을 하는 기능
    - [x] 크루원 배열을 섞는 기능
    - [x] 앞에서 부터 2명씩 엮는 기능 / 크루원이 홀수면 마지막에 포함
    - [ ] 이미 패어가 된적이 있으면 다시 재 매칭
      - [ ] 횟수를 측정해서 3회 이상 넘어가면 ERROR 출력
    - [x] 해당 레벨에 매칭이력이 있는지 확인하는 기능
    - [x] 크루원의 매칭을 하는 기능
  - [ ] 매칭을 조회하는 기능
    - [ ] 매칭이력이 있는지 확인하는 기능
    - [ ] 없으면 ERROR 출력
